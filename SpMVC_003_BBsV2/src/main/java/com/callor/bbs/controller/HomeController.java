...
public class HomeController {
	 ...
	 *  2개이상의 Component 들이 같은 Service interface를 상속받아 생성된 경우,
	 * 이 Component를 주입받아 사용하는 곳에서는 어떤 Component 를 주입받아 
	 * 사용해야 하는지 알수가 없다. 
	 * 
	 * 따라서 이때는 @Component("이름") 에 이름을 부여하고 
	 * 사용할 곳에서 주입받을때 @Quakifier()를 통해 명시적으로 어떤
	 * Component를 주입 받을지 지정해 주어야 한다. 
	 */
...
//		List<BBsDto> bbsList = bbsDao.selectAll();
	...
	/*
	 * @ModelAttribute("BBS")
	 * GET /insert 가 호출될때 아직 bbsDto 객체는 null 인 상태이다.
	 * bbsDto 가 null 인경우 
	 * 어딘가에 @ModelAttribute("BBS") 라는 이름이 부착된 method를 찾는다.
	 * 만약 method 가 발견되면 자동으로 해당 method 를 호출하여 데이터를 준비해 준다. 
	 * 
	 */
	 ...
	 * Client(Browser)에서 b_username 이라는 변수에 값이 담겨서 전달 되어 오면,
	 * username이라는 변수에 값을 저장하여 method 내부로 전달하라.
	 * 
	 * Client 와 Server 간에 변수 이름일 다를때 사용하는 선택사항
	 */
	@RequestMapping(value = "/insert", method = RequestMethod.POST)
	/*
	 * < 매개변수 설명 >
	 * BBsDto BBsDto : input tag에 입력한 게시판관련 text 를 받는 곳.
	 * MultipartFile b_file : input tag 의 type="file"에서 선택한 파일을 받는곳.
	 */
	public String insert(BBsDto bbsDto,
		...
		log.debug("사용자이름 : {b_username} ",bbsDto.getB_username());
		...
		try {
			...
			// 멀티파일 업로드가 실행 되었을때만 filesUp() 호출.
			// getFile().getSize() : b_image 를 List 로 변환하고 개수새기
...
//		return "input";
	...
	public BBsDto getBBsDto() {
		...
		bbsDto.setB_username("yu1128eun");
		return bbsDto;
	}
