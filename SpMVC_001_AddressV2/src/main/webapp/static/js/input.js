...
document.addEventListener("DOMContentLoaded", () => {
   ...
   *  1. 변수1의 값이 false 가 아니면 변수 1의 값을 왼쪽 변수에 대입.
   *  2. 변수2의 값이 false 이면 변수2의 값을 비교
   *  3. 변수2의 값이 false 가 아니면 변수 2의 값을 왼쪽 변수에 대입.
   *  4. 변수4까지 (변수1~변수4) 모든 변수가 false 인 값을 가지고 있으면,
   *  5. 왼쪽 변수는 없음 이라는 문자열이 대입된다.
   *
   * let 변수 = null;
   * if(변수1) 변수 = 변수1
   * else if(변수2) 변수 = 변수2
  ...
  // event call 함수를 선언하고
  const inputButtonClickHander = (e) => {
    // alert("추가 버튼 클릭");
    // JS 코드에서 form에 포함된 button 을 클릭 했을때
    // Server로 데이터를 전송하라
    // input_id = document.querySeletor("form.main input[name='a_id']");
    // 위의 코드를 두개로 나눈것이 아래이다.
  ...
  // 선언된 event call함수를 사용하여 click event 선언
  btn_input?.addEventListener("click", inputButtonClickHander);
  ...
  input_id?.addEventListener("blur", async (e) => {
    ...
    if (result === "OK") {
      ...
      message_view(INDEX.ID, "error", " * 이미 추가된 ID입니다.");
      input_id.focus();
    } else {
      message_view(INDEX.ID, "error", " * 서버오류, 알수업는 오류입니다.");
      input_id.focus();
    }
  });
});
