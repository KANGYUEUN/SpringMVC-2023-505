...
public class HomeController {
	...
	public String home(@ModelAttribute("CAR") CarDto carDto) {
		log.debug("나는 HOMECONTROLLER ");
		return "car/input";
	}
	
	@RequestMapping(value = "/input", method=RequestMethod.POST)
	public String input(@ModelAttribute("CAR")
					CarDto carDto) {
	 ...
	 * 데이터를 있는 그래도 Client 에게 보내라. 
	 * CarDto 객체에 담긴 데이처를 JSON 객체로 변환하여 Client 로 전송하라
	...
	public CarDto carDto() {
		...
		CarDto carDto = CarDto.builder().c_sdate(strDate).c_stime(strTime)
							.c_username("yu1128eun").c_qty("시작").c_carnum("111가1234")
							.build();
		return carDto;
	}
